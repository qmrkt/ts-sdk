...
import { type ClientConfig, type MethodCallOptions, loadMethods, callMethod, readGlobalState, readBox, boxName, boxNameAddr, boxNameAddrIdx, marketBoxRefs, bootstrapBoxRefs, pricingBoxRefs, ceilDiv, withMinFlatFee, textEncoder, MIN_TXN_FEE, SECONDS_PER_DAY } from '../base.js'
...
export function assertWholeShareMultiple(shares: bigint, action: 'buy' | 'sell' | 'claim' | 'refund'): void {
...
export function requiredBondFromState(
  minimum: bigint,
  bps: bigint,
  cap: bigint,
  poolBalance: bigint,
...
export function assertActiveLpSkewWithinCap(state: Pick<MarketState, 'prices' | 'lpEntryMaxPriceFp'>): void {
 ...
 */
export function makeAssetTransfer(
  sender: string,
  receiver: string,
  assetId: number,
  amount: bigint,
 ...
 */
export function deduplicateBoxes(refs: algosdk.BoxReference[]): algosdk.BoxReference[] {
...
export function decodeAddressStateValue(value: bigint | Uint8Array | undefined): string | undefined {
...
export function stateValue<T>(state: Record<string, unknown>, ...keys: string[]): T | undefined {
...
export async function getLatestBlockTimestamp(algod: algosdk.Algodv2): Promise<number> {
...
export function payoutBoxRefs(appId: number, ...accounts: Array<string | undefined>): algosdk.BoxReference[] {
  return accounts
...
export async function buildAsaOptInIfNeeded(
...
export async function buildAppOptInIfNeeded(
  algod: algosdk.Algodv2,
  sender: string,
  signer: algosdk.TransactionSigner,
  appId: number | bigint,
 ...
 */
export function noopsFor(numOutcomes: number): number {
...
async function buildBudgetedCall(
  ...
  for (let i = 0; i < marketNoopCount; i++) {
    ...
    atc.addTransaction({
      txn: makeBareAppNoOp(config.sender, Number(config.appId), sp, {
        ...
        note: textEncoder.encode(`n${methodName}${i}`),
      }),
  ...
  for (let i = 0; i < extraMarketNoopCount; i++) {
    atc.addTransaction({
      txn: makeBareAppNoOp(config.sender, Number(config.appId), sp, {
        note: textEncoder.encode(`p${methodName}${i}`),
      }),
      signer: effectiveSigner,
    })
  }
  ...
  if (opts?.budgetAppId && budgetOpUpCount > 0) {
    ...
    atc.addMethodCall({
      ...
      note: textEncoder.encode(`opup:${methodName}:${budgetOpUpCount}`),
    })
  }
...
export async function getProtocolConfigAppId(
  algod: algosdk.Algodv2,
  appId: number | bigint,
...
export function getProtocolBudgetForeignApps(
...
export async function storeMainBlueprint(config: ClientConfig, data: Uint8Array) {
  return callWithBudget(config, 'store_main_blueprint', [data], 2, 0, 10, {
    extraBoxes: [{ appIndex: Number(config.appId), name: textEncoder.encode('mb') }],
  })
}
...
export async function storeDisputeBlueprint(config: ClientConfig, data: Uint8Array) {
  return callWithBudget(config, 'store_dispute_blueprint', [data], 2, 0, 10, {
    extraBoxes: [{ appIndex: Number(config.appId), name: textEncoder.encode('db') }],
  })
}
...
export async function bootstrap(
  ...
  await storeResolutionLogic(config, textEncoder.encode('default'))
  ...
  for (const [index, boxes] of noopBoxChunks.entries()) {
    atc.addTransaction({
      txn: makeBareAppNoOp(config.sender, Number(config.appId), suggestedParams, {
        note: textEncoder.encode(`npost_comment${index}`),
        boxes,
      }),
