...
describe("Frontend spec drift guard", () => {
  it("frontend QuestionMarket.arc56.json matches compiled artifact", () => {
  ...
  it("frontend MarketFactory.arc56.json matches compiled artifact", () => {
    const compiled = loadSpec(
  ...
  it("sdk QuestionMarket.arc56.json matches compiled artifact", () => {
    const compiled = loadSpec(
      path.join(ARTIFACTS, "market_app/QuestionMarket.arc56.json")
    );
  ...
  it("sdk MarketFactory.arc56.json matches compiled artifact", () => {
    const compiled = loadSpec(
      path.join(ARTIFACTS, "market_factory/MarketFactory.arc56.json")
    );
  ...
  it("frontend ProtocolConfig.arc56.json matches compiled artifact", () => {
    const compiled = loadSpec(
  ...
  it("sdk ProtocolConfig.arc56.json matches compiled artifact", () => {
    const compiled = loadSpec(
      path.join(ARTIFACTS, "protocol_config/ProtocolConfig.arc56.json")
    );
  ...
  it("frontend compiled-programs.json matches compiled artifact bytecode", () => {
  ...
  it("chain-client.ts references shared market box constants", () => {
  ...
  it("bootstrapBoxRefs includes shared totals and blueprint boxes", () => {
    const chainSrc = fs.readFileSync(FRONTEND_CHAIN_CLIENT, "utf8");
  ...
  it("tradeBoxRefs includes total-user-shares box", () => {
    const chainSrc = fs.readFileSync(FRONTEND_CHAIN_CLIENT, "utf8");
  ...
  it("create-market.ts delegates UI creation to the atomic SDK path only", () => {
  ...
  it("executeWithBudget auto-injects prependTxns as axfer method args", () => {
    const chainSrc = fs.readFileSync(FRONTEND_CHAIN_CLIENT, "utf8");
  ...
  it("noopCount default is at least 14 for opcode budget", () => {
    const chainSrc = fs.readFileSync(FRONTEND_CHAIN_CLIENT, "utf8");
  ...
  it("create-market.ts does not hand-roll factory ABI args or standalone prerequisite txns", () => {
    const createSrc = fs.readFileSync(FRONTEND_CREATE, "utf8");
  ...
  it("frontend create-market preserves ARC-2 metadata note for atomic flow", () => {
    const createSrc = fs.readFileSync(FRONTEND_CREATE, "utf8");
    expect(createSrc).toContain("question.market:j");
    expect(createSrc).toContain("noteBytes");
  });

  it("lifecycle-e2e uses the current create_market ABI and no removed dispute hook", () => {
    const spec = loadSpec(
      path.join(FRONTEND_CONTRACTS, "MarketFactory.arc56.json")
    );
...
describe("SDK noop budget and resolution method validation", () => {
  const SDK_QUESTION_MARKET = path.resolve(
    ROOT,
    "src/clients/question-market.ts"
  );

  it("frontend executeWithBudget default noop count is >= 14", () => {
    const chainSrc = fs.readFileSync(FRONTEND_CHAIN_CLIENT, "utf8");
    const match = chainSrc.match(/noopCount\s*\?\?\s*Math\.max\((\d+)/);
  ...
  it("SDK buy uses sufficient noops (scales with outcomes)", () => {
    const src = fs.readFileSync(SDK_QUESTION_MARKET, "utf8");
    ...
    const buyFn = extractFunctionBody(src, "buy");
  ...
  it("SDK tradeBoxRefs includes shared total-user-shares box for trade calls", () => {
